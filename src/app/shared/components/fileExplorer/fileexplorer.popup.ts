...
export class FileExplorerPopupPage extends PopupBasePage implements OnInit, AfterViewInit {
    ...
    @ViewChild('lov') lov: DXLovComponent;
    ...
    getImageStyle(item) {
        ...
        return {
            'background': `url('${this.getImage(url)}')`,
        ...
        switch (name) {
            ...
            case "CreateFolder":
                this.popup.open(FolderPopupComponent, {
                    ...
                    height: '200',
                    width: '400',
                    data: { parentFolderId: this.folderid }
                }).then(res => {
    ...
    deleteFile(id) {
        this.service.postPromise('/EDM/File/Delete', id).then(res => {
            Notify.success('فایل با موفقیت حذف شد')
        }).catch(err => {
            Notify.success('عملیات با خطا مواجه شد')
        });
    }
