...
export class DXLovComponent {
  ...
  @ViewChild("comboBox") comboBox: DxLookupComponent;
  ...
  //*********************************************
  @Output() popupWidthChange: EventEmitter<number> = new EventEmitter<number>();
  private _popupWidth: number = 600;
  @Input()
  get popupWidth(): number {
    return this._popupWidth;
  }
  set popupWidth(val: number) {
  ...
  //*********************************************
  @Output()
  popupHeightChange: EventEmitter<number> = new EventEmitter<number>();
  private _popupHeight: number = 700;
  @Input()
  get popupHeight(): number {
    return this._popupHeight;
  }
  set popupHeight(val: number) {
  ...
  getFormList(params): Promise<any> {
    let that = this;
    return this.service.getPromise('/SYS/Forms/List', params, { loading: true }).then(res => {
  ...
  ngAfterViewInit() {
    if (this.textBox) {
      let validator = this.textBox.validator;
      if (this.required) {
    ...
    if (this.comboBox) {
      let validator = this.comboBox.validator;
      if (this.required) {
        validator.instance.option("validationRules", [
          { type: "required", message: "این فیلد اجباری است" }
        ]);
  ...
  private onKeyDown(e) {
    ...
    if (e.event.key == "Enter") {
      ...
      let that = this;
      setTimeout(() => {
        // console.log("findBySearch", filter);
