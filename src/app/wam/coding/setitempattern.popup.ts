...
//
@Component({
    selector: 'wam-setitempattern-popup',
    templateUrl: './setitempattern.popup.html'
})
export class WAMSetItemPatternPopupPage extends BasePage {
    // Visible
    @Output()
    ...
    // ItemPatternValues
    @Output()
    ItemPatternValuesChange: EventEmitter<any> = new EventEmitter<any>();

    _ItemPatternValues: any;
    @Input()
    get ItemPatternValues(): any {
        return this._ItemPatternValues;
    }

    set ItemPatternValues(val: any) {
        this._ItemPatternValues = val;
        this.ItemPatternValuesChange.emit(this._ItemPatternValues);
    }
    ...
    bindData() {
        ...
        if (this.ItemPatternValues == null || this.ItemPatternValues.length == 0) {
            ...
            this.service.get("/WAM/CodingPattern/List", (data) => {
                ...
                for (item in t) {
                    ...
                    t[item].CollumnText = this.ItemPatternValues[item].Value;
                }
    ...
    onSaveClick() {
        ...
        if (!nullValues) {
            if (this.saveFlag) {
                this.service.post("/WAM/Item/CreateCode", (data) => {
                    this.code = data;
                    this.ItemPatternValues = this.patternValues;
                    //this.
                    this.visible = false;
                }, this.patternValues);
            }
